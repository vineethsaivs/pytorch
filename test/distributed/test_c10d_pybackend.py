...
class TestPyBackend(TestCase):
    def test_attr_overrides(self) -> None:
        ...
        self.assertEqual(backend.name(), "python-backend")
        ...
        for attr in (
            ...
            self.assertTrue(getattr(C10DBackend, attr).__get__(backend, type(backend)))

        options = C10DBackend.options.__get__(backend, type(backend))
        self.assertEqual(options.backend, "python-backend")

        backend._set_group_name("name")
        backend._set_group_desc("desc")
        self.assertEqual(backend.group_name, "name")
        self.assertEqual(backend.group_desc, "desc")

        backend.bound_device_id = torch.device("cpu:0")
        self.assertEqual(backend.bound_device_id, torch.device("cpu:0"))

        backend.use_pg_for_symm_mem_rendezvous = True
        self.assertTrue(backend.use_pg_for_symm_mem_rendezvous)

    def test_collective_overrides(self) -> None:
        backend = RecordingBackend(0, 1)

        allreduce_tensor = torch.zeros(2)
        work = C10DBackend.allreduce(backend, [allreduce_tensor])
        self.assertTrue(work.wait())
        self.assertEqual(allreduce_tensor, torch.full((2,), 2.0))

        broadcast_tensor = torch.zeros(2)
        C10DBackend.broadcast(backend, [broadcast_tensor]).wait()
        self.assertEqual(broadcast_tensor, torch.ones(2))

        output = torch.zeros(2)
        input = torch.ones(2)
        C10DBackend.all_gather_single(backend, output, input).wait()
        ...
        C10DBackend.all_gather_single_coalesced(backend, outputs, inputs).wait()
        self.assertEqual(outputs, inputs)
        self.assertEqual(backend.calls[-1][0], "all_gather_single_coalesced")

        rs_output = torch.zeros(2)
        rs_input = torch.arange(2.0)
        C10DBackend.reduce_scatter_single(backend, rs_output, rs_input).wait()
        ...
        C10DBackend.reduce_scatter_single_coalesced(
            backend, rs_outputs, rs_inputs
        ).wait()
        ...
        C10DBackend.all_to_all_single(backend, a2a_output, a2a_input, [], []).wait()
        self.assertEqual(a2a_output, a2a_input)
        self.assertEqual(backend.calls[-1][0], "all_to_all_single")

        recv_any = torch.zeros(2)
        C10DBackend.recv_anysource(backend, [recv_any], 0).wait()
    ...
    def test_backend_only_overrides(self) -> None:
        ...
        C10DBackend.set_timeout(backend, timeout)
        self.assertEqual(backend.calls[-1], ("set_timeout", timeout))

        shrunk = C10DBackend.shrink(backend, [1], 7, None)
        self.assertEqual(shrunk.name(), "shrunk-python-backend")
        self.assertEqual(backend.calls[-1], ("shrink", [1], 7, None))

        self.assertEqual(
            C10DBackend.get_reconfigure_handle(backend),
            "handle-for-python-backend",
        )
        opts = ReconfigureOptions()
        C10DBackend.reconfigure(backend, opts).wait()
        self.assertIs(backend.reconfigure_opts, opts)

        C10DBackend._start_coalescing(backend)
        C10DBackend._end_coalescing(backend).wait()
        self.assertEqual(backend.start_coalescing_count, 1)
        self.assertEqual(backend.end_coalescing_count, 1)

        C10DBackend._set_sequence_number_for_group(backend)
        self.assertEqual(C10DBackend._get_sequence_number_for_group(backend), 123)

        C10DBackend.monitored_barrier(backend, timeout, True)
        self.assertEqual(backend.calls[-1][0], "monitored_barrier")

        C10DBackend._register_on_completion_hook(backend, lambda work_info: None)
        self.assertTrue(C10DBackend._has_hooks(backend))
        self.assertIsNotNone(backend.registered_hook)

        C10DBackend._wait_for_pending_works(backend)
        C10DBackend._enable_collectives_timing(backend)
        ...
        split = C10DBackend.split(backend, store, [0], split_opts)
        self.assertEqual(split.name(), "split-backend")

        merge = C10DBackend.merge(backend, store, split_opts, 0, 1)
        self.assertEqual(merge.name(), "merged-backend")

        device = torch.device("cpu:0")
        C10DBackend.eager_connect_single_device(backend, device)
        self.assertEqual(backend.eager_device, device)

        self.assertEqual(C10DBackend.get_error(backend), ErrorType.SUCCESS)
        self.assertTrue(C10DBackend.supports_tensor_alloc(backend, device))
        self.assertEqual(backend.tensor_alloc_device_idx, 0)

        allocated = C10DBackend.allocate_tensor(
            backend,
            4,
            dtype=torch.float64,
        ...
        C10DBackend.suspend(backend)
        C10DBackend.resume(backend)
        self.assertTrue(backend.suspended)
        self.assertTrue(backend.resumed)
        self.assertEqual(C10DBackend.memory_stats(backend), {"allocated": 7})

        C10DBackend.abort(backend)
        C10DBackend.shutdown(backend)
