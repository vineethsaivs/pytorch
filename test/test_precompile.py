...
class TestPrecompile(TestCase):
    ...
    # Input mutation, output aliasing, and tensor subclasses are SUPPORTED: the
    # inductor backend lowers through aot_autograd.compile_to_python, which composes
    # AOTAutograd's own codegen'd prelude/epilogue into the artifact. Only
    # functionalized RNG and effectful ops are rejected up front; anything the
    # composition cannot reproduce is caught by a correctness self-check (raising
    # rather than emitting a silently-wrong artifact, e.g. the synthetic-base case).

    def test_output_alias_supported(self):
    ...
    def test_input_mutation_supported(self):
        ...
        code, cache = torch.precompile(lambda a: a.add_(1.0), torch.randn(4))
        f_c = torch.precompile.load(code, cache)
        x = torch.zeros(4)
        out = f_c(x)
        self.assertEqual(x, torch.ones(4))
        self.assertEqual(out, torch.ones(4))

    def test_functionalized_rng_supported(self):
    ...
    def test_mutated_duplicate_input(self):
        ...
        # artifact reproduces eager when run with the same aliasing. (A genuine
        # runtime synthetic-base divergence would be caught by the composition's
        # correctness self-check, which raises rather than emit a wrong artifact.)
    ...
    def test_batchnorm_train_buffer_mutation(self):
        # End-to-end: a stateful module (BatchNorm in training mode) mutates its
        # running stats. precompile (inductor backend) reflects that onto the runtime
        # model's buffers and matches eager -- the mutation handling comes from
        # AOTAutograd's codegen composed into the artifact.
        def fresh():
            torch.manual_seed(0)
    ...
    def test_backend_eager_no_inductor_lowering(self):
        ...
        # eager driver. The cache carries the GraphModule, not an inductor artifact.
        m = torch.nn.Sequential(torch.nn.Linear(4, 3)).eval()
        x = torch.randn(5, 4)
        ...
        # The cache carries ONLY the captured GraphModule as the artifact; the
        # backend tag lives in python_code (the single source of truth), not the cache.
        self.assertIn("BACKEND = 'eager'", code)
        blob = torch.load(io.BytesIO(cache), weights_only=False)
        self.assertEqual(set(blob), {"artifact"})
        self.assertIsInstance(blob["artifact"], torch.fx.GraphModule)
