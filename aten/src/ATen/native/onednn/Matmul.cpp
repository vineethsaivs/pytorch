...
onednn_gemm(
  ...
  bool tf32_usable = std::is_same_v<scalar_t, float> && use_onednn_tf32_matmul();
...
void onednn_matmul(
  ...
  if (mat1.scalar_type() == at::kHalf) {
    TORCH_CHECK(
        onednn_fp16_device_check_arm(),
        "onednn_matmul: onednn_matmul fp16 path needs a cpu with fp16 support");
  }
#else
  TORCH_CHECK(
      (mat1.scalar_type() == at::kBFloat16 ||
  ...
  bool bf32_usable = mat1.scalar_type() == at::kFloat && use_onednn_bf32_matmul();
  bool tf32_usable = mat1.scalar_type() == at::kFloat && use_onednn_tf32_matmul();
...
static inline bool checksize(const Tensor& mat1, const Tensor& mat2){
  ...
  constexpr int64_t onednn_gemm_min_size = 16 * 16 * 16;
  if (mat1.dim() == 1 && mat2.dim() == 1) {
    // aten::dot
...
bool use_onednn_bf16_matmul(
  ...
  if (onednn_bf16_device_check_arm()) {
    ...
    // Arm Neoverse V1 so, don't restrict the onednn_matmul only for bf16
  ...
  {
    return (
        use_onednn_bf16_matmul() && mat1.scalar_type() == kBFloat16 &&
...
bool use_onednn_fp16_matmul(
    ...
    const Tensor& result) {
  return (
      use_onednn_fp16_matmul() && mat1.scalar_type() == kHalf &&
...
bool use_onednn_bf32_matmul(
    ...
    const Tensor& result) {
  return (
      use_onednn_bf32_matmul() && mat1.scalar_type() == kFloat &&
      mat2.scalar_type() == kFloat &&
      (!result.defined() || result.scalar_type() == kFloat) &&
      mat1.numel() != 0 && mat2.numel() != 0 && checksize(mat1, mat2));
}

bool use_onednn_tf32_matmul(
    const Tensor& mat1,
    const Tensor& mat2,
    const Tensor& result) {
  return (
      use_onednn_tf32_matmul() && mat1.scalar_type() == kFloat &&
      mat2.scalar_type() == kFloat &&
      (!result.defined() || result.scalar_type() == kFloat) &&
      mat1.numel() != 0 && mat2.numel() != 0 && checksize(mat1, mat2));
}
...
bool use_onednn_matmul(
    ...
    const Tensor& result) {
  return (
      use_onednn_bf16_matmul(mat1, mat2, result) ||
      use_onednn_fp16_matmul(mat1, mat2, result) ||
      use_onednn_bf32_matmul(mat1, mat2, result) ||
      use_onednn_tf32_matmul(mat1, mat2, result));
}
...
void onednn_matmul_i8i8i32(
  ...
  if (a_is_contigous && b_is_contigous) {
    if (mat1.scalar_type() == at::kByte) {
      _onednn_gemm_i8i8i32_with_blas<uint8_t>(mat1, mat2, result);
    } else {
      _onednn_gemm_i8i8i32_with_blas<int8_t>(mat1, mat2, result);
    }
  } else {
