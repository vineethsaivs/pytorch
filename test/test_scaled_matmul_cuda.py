...
class TestFP8Matmul(TestCase):
    ...
    def test_scaled_mm_vs_emulated_row_wise(self, base_dtype, shapes, device):
        ...
        # Mirrors the ScaledBlas.cpp routing to identify the CUTLASS fallback, which
        # needs a looser fp32 element-wise tolerance.
        cutlass_rowwise = (
            ...
            and (
                ...
                or (
                    ...
                    and (x_scales.dim() > 0 or y_scales.dim() > 0)
                )
            )
        )
        ...
        def test():
            ...
            if base_dtype in {torch.bfloat16, torch.float16}:
                atol, rtol = 7e-2, 7e-2
            elif cutlass_rowwise:
                # CUTLASS fp32 can differ from the emulated reference in accumulation order;
                # keep cosine strict while allowing a looser element-wise tolerance.
                atol, rtol = 5e-3, 5e-3
            else:
                atol, rtol = 2e-3, 2e-3
