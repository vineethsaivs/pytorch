...
class VariableTracker(metaclass=VariableTrackerMeta):
    ...
    def call_getattribute(
        ...
        getattro_impl, which is correct because the base call_getattr_fallback
        is a no-op.  UDOV overrides to handle custom __getattribute__ and
        to call generic_getattr with skip_getattr_fallback=True."""
    ...
    def call_method(
        ...
        if name == "__getitem__":
            ...
            raise_observed_exception(
                AttributeError,
                tx,
                args=[f"'{type_name}' object has no attribute '{attr_name}'"],
            )
        elif (
