...
SECURITY: the ``cache`` is loaded with ``torch.load(..., weights_only=False)`` and
therefore unpickles arbitrary Python objects (the compiled artifact). Loading a
cache executes code embedded in it, exactly like unpickling. Only load caches you
produced or otherwise trust (invariant 7).
"""
...
#    model is undefined. PARTIALLY enforced: the inlined path checks the dense input
#    count, but a same-count-different-structure model is not detected.
#
...
#    reason; this is an implementation gap, not a fundamental limit. Separately,
#    anything the composition cannot reproduce (e.g. a runtime wrapper that is a plain
#    closure rather than codegen'd source, like FakifiedOutWrapper) is caught by a
#    correctness self-check that raises rather than emit a silently-wrong artifact.
#
...
#    a ``zero_grad()`` / ``optimizer.step()`` loop works unchanged. The artifact
#    therefore returns ``fn``'s own result (``None`` for a bare ``.backward()`` step),
#    not the grads. The grad scatter is the ONLY mutation precompile performs, and it
#    happens in Python outside the graph, so the graph stays functional (invariant 4
#    is about in-graph mutation and is unaffected). precompile does not own optimizer
#    state; bring your own optimizer and zero grads as usual.
#
...
#
# 7. The cache is trusted, executable state. load() unpickles it with
#    weights_only=False; treat a cache like code you are about to run.
#
...
# calling convention. The ``cache`` holds ONLY the compiled/captured artifact and is
...
# to reconstruct the real compiled artifact (inductor; FxGraphCache hit, no JIT) or
# the pickled GraphModule (eager). Same results either way -- with the cache you skip
# JIT; with only python_code you JIT. The metadata lives in one place: no drift.
#
...
# backends -- the same graph is captured; only its realization differs. One
# mechanical consequence: the eager backend runs the graph directly on the
...
def _scatter_grads(params: list[Any], grads: list[Any]) -> None:
    ...
    ``grads`` is in lifted-param order, so ``grads[i]`` belongs to ``params[i]`` (the
    interned unique params). Accumulating rather than overwriting means a standard
    ``zero_grad()`` / ``optimizer.step()`` loop behaves as it does in eager.
    """
...
def _capture(
    ...
    def flat_fn(flat: list[Any]) -> list[Any]:
        ...
        # the runtime model's .grad fields. grad_flat is empty when fn ran no
        # backward; otherwise it has one (zero-filled) grad per unique param, in
        # param order, so grad i corresponds to lifted param input i.
    ...
    # is non-mutating, and the runtime driver accumulates onto whatever .grad it finds.
    # Snapshot and restore the example params' grads around the trace.
...
def _parse_artifact_metadata(python_code: str) -> dict[str, Any]:
    ...
    wanted = {"BACKEND", "MODULE_POSITIONS", "NUM_GRAD_OUTPUTS", "OUT_SPEC"}
    found: dict[str, Any] = {}
...
# and the executable code. The companion ``cache`` returned by precompile is purely
# an acceleration used by torch.precompile.load (it runs the pickled GraphModule
# directly); this file does not read it.
"""
...
def _build_eager_python_source(compiled: PrecompiledModule) -> str:
    ...
    parts.append("import torch.fx._pytree as fx_pytree")
    parts.append("import torch.utils._pytree as pytree")
...
def forward(*args):
    ...
    (accumulated) onto the runtime model's .grad fields like eager .backward()."""
    mods = [args[i] for i in MODULE_POSITIONS]
    ...
    if NUM_GRAD_OUTPUTS:
        grads = out[len(out) - NUM_GRAD_OUTPUTS:]
        out = out[:len(out) - NUM_GRAD_OUTPUTS]
...
def forward(*args):
    ...
    outputs are parameter grads: they are scattered (accumulated) onto the runtime
    model's .grad fields, mirroring eager .backward(), and the artifact returns fn's
    own result. Nothing here reads an external cache: the kernels JIT-compile from
    the inlined source on first call."""
    mods = [args[i] for i in MODULE_POSITIONS]
    user_inputs = [a for i, a in enumerate(args) if i not in set(MODULE_POSITIONS)]
    user_flat, _ = _pytree.tree_flatten(tuple(user_inputs))
    pb = _extract_param_buffers(mods)
...
class PrecompiledModule:
    ...
    def to_cache_bytes(self) -> bytes:
        ...
        the calling convention, so the cache holds ONLY the compiled/captured
        artifact -- no calling-convention metadata, no model weights. load() recovers
        the calling convention by parsing python_code (``_parse_artifact_metadata``)
        and uses this artifact solely to skip JIT/recompile. For the inductor backend
        the artifact is the real Inductor/AOTAutograd compiled-artifact bytes (load
        primes the inductor cache via a FxGraphCache hit, so reload does not
        re-trace/re-lower and, on GPU, restores bundled Triton kernels); ``None`` if
        the graph is uncacheable, in which case load() falls back to the standalone
        python. For the eager backend it is the captured GraphModule, run as-is.
        """
        if self._backend == "eager":
            # The captured ATen graph is the artifact; it pickles cleanly and is run
            # as-is on load (no Inductor lowering).
            artifact: Any = self._gm
        else:
            # The opaque artifact-cache bytes from aot_autograd.compile_to_python
            # (None for an uncacheable graph; load() then falls back to executing the
            # self-contained python).
            artifact = self._artifact_bytes
        buf = io.BytesIO()
        torch.save({"artifact": artifact}, buf)
        return buf.getvalue()
...
def _make_eager_forward(
    ...
    """Drive the captured ATen graph (eager backend) directly.

    The graph runs as captured: the runtime model(s)' params/buffers and the user
    inputs are passed through unchanged (no Inductor lowering, no dense
    flatten/unflatten), and the graph's flat outputs are reassembled into fn's
    output structure. The runtime model must match the example model structurally
    (invariant 2 of Note [precompile programming model]).
    """
    pos = set(module_positions)
    ...
    def forward(*args: Any) -> Any:
        ...
        user_flat, _ = pytree.tree_flatten(user_inputs)
        with torch.no_grad():
            out = gm([*pb_flat, *user_flat])
        out_list = list(out) if isinstance(out, (list, tuple)) else [out]
        if num_grad_outputs:
            grads = out_list[len(out_list) - num_grad_outputs :]
            out_list = out_list[: len(out_list) - num_grad_outputs]
            _scatter_grads(pb_flat, grads)
        return pytree.tree_unflatten(out_list, out_spec)
...
class _PrecompileApi:
    ...
    def __call__(
        ...
        *args: Any,
        ...
        - ``"eager"``: do NOT lower -- keep the captured ATen graph and run it as-is
          (analogous to ``torch.compile(backend="eager")``). ``python_code`` renders
          the readable captured graph for inspection; the cache holds the
          GraphModule, which ``load`` runs directly. Useful for inspecting/debugging
          exactly what was traced without an Inductor dependency.
    ...
    def load(self, python_code: str, cache: bytes) -> PrecompiledModule:
        ...
        captured artifact and is a pure acceleration (Note [precompile programming
        model], "self-contained"): for the eager backend it is the captured
        GraphModule, run directly; for the inductor backend it is the serialized
        ...
        AOTAutograd runtime, so tensor subclasses (DTensor) work. With no artifact in
        the cache, exec'ing ``python_code`` JITs the kernels (inductor) or runs the
        inlined graph (eager) -- same result, just without the cache's speedup.
        ...
        SECURITY: this unpickles ``cache`` with ``weights_only=False`` and so
        executes arbitrary code embedded in it. Only load trusted caches
        (invariant 7).
        """
        ...
        # The cache carries only the compiled/captured artifact: use it for the fast
        # reconstruction, else fall back to exec'ing the self-contained python_code.
        blob = torch.load(io.BytesIO(cache), weights_only=False)
        artifact = blob.get("artifact")
        gm = None
        if backend == "eager" and artifact is not None:
            gm = artifact
            forward = _make_eager_forward(
                gm, module_positions, out_spec, num_grad_outputs
            )
        elif backend != "eager" and artifact is not None:
            forward = _make_cached_forward(
                artifact, module_positions, out_spec, num_grad_outputs
            )
        else:
            forward = _make_inlined_forward(python_code)

        obj = PrecompiledModule.__new__(PrecompiledModule)
        obj._fn = None  # type: ignore[assignment]
        obj._backend = backend
        obj._decompositions = None
        obj._artifact = None
        obj._graph_python = ""
        obj._artifact_bytes = None
        obj._module_positions = module_positions
        obj._num_grad_outputs = num_grad_outputs
        obj._param_names = []
        obj._buffer_names = []
        obj._num_params_buffers = 0
        obj._in_spec = None
        obj._out_spec = out_spec
        # Keep the captured graph reachable for inspection (eager backend).
        obj._gm = gm
